...
- **Word → JPG 변환** — .docx 각 페이지를 JPEG 이미지로 렌더링
- **Excel → JPG 변환** — .xlsx 각 워크시트를 JPEG 이미지로 렌더링
- **Markdown → JPG 변환** — .md를 모바일 친화적인 9:16 JPEG (2K 해상도)로 변환
...
| Word (.docx) | JPG (페이지별) | 각 페이지를 개별 이미지로 렌더링 |
| Excel (.xlsx) | JPG (시트별) | 각 워크시트를 개별 이미지로 렌더링 |
| Markdown (.md) | JPG | 단일 또는 다중 페이지 (9:16 비율, 2K 해상도) |
...
FinderBridge ──> AppleScript ──> Finder 선택 항목 [URL]
    |
    v
FileRouter ──> 매직 바이트 + ZIP 구조로 분류
    |
    v
ImageProcessor / PDFProcessor / WordProcessor
ExcelProcessor / MarkdownProcessor ──> _opt.jpg 출력
    |
    v (메인 스레드)
